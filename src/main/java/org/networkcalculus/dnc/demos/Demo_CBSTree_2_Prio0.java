...
public class Demo_CBSTree_2_Prio0 {

    public Demo_CBSTree_2_Prio0() {
    }

    public static void main(String[] args) {
        Demo_CBSTree_2_Prio0 demo = new Demo_CBSTree_2_Prio0();
    ...
    public void run() throws Exception {
        ...
        ArrivalCurve ac_flow1 = Curve.getFactory().createTokenBucket(6.336e6, 5934.55104);
        ...
        servers[0] = sg.addServer("S0", Curve.getFactory().createRateLatency(2.5e7, 1.2336E-4), AnalysisConfig.Multiplexing.FIFO);
        servers[1] = sg.addServer("S1", Curve.getFactory().createRateLatency(2.5e7, 1.2336E-4), AnalysisConfig.Multiplexing.FIFO);
        servers[2] = sg.addServer("S2", Curve.getFactory().createRateLatency(2.5e7, 1.2336E-4), AnalysisConfig.Multiplexing.FIFO);
        servers[3] = sg.addServer("S3", Curve.getFactory().createRateLatency(2.5e7, 1.2336E-4), AnalysisConfig.Multiplexing.FIFO);

        /* Define links between server */
        Turn t_0_1 = sg.addTurn("S0 --> S1", servers[0], servers[1]);
        Turn t_1_3 = sg.addTurn("S1 --> S3", servers[1], servers[3]);
        Turn t_2_3 = sg.addTurn("S2 --> S3", servers[2], servers[3]);

        /* Define path for flow 0,1 */
        LinkedList<Turn> path0 = new LinkedList<Turn>();
        path0.add(t_0_1);
        path0.add(t_1_3);

        sg.addFlow("Flow 0", ac_flow1, path0);
        ...
        sg.addFlow("Flow 2", ac_flow1, path1);
